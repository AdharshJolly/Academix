    ...
    def sync_schedules_background(self, user_id: str, schedules: List[ScheduleBlock]) -> None:
        ...
        for date_str, blocks in schedules_by_date.items():
            try:
                ...
                already_scheduled: list[Tuple[datetime, datetime]] = []
                
                for block in blocks:
                    duration_hours = float(block.duration_hours)
                    ...
                    self.calendar_client.create_timed_event(
                        refresh_token=refresh_token,
                        title=f"{block.session_type.capitalize()}: {block.subject}",
                        event_date=date_str,
                        duration_hours=duration_hours,
                        start_time=start_time,
                        description=f"Generated by Academix AI.\nSession Type: {block.session_type}"
                    )
            except Exception as e:
